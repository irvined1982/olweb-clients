...
def print_long():
    for queue in Queue.get_queues_by_names(connection, args.queue_names):
        ...
        print "%-4.4s %-4.4s %-15.15s %-3.3s %-4.4s %-4.4s %-4.4s %-6.6s %-6.6s %-3.3s %-5.5s %-6.6s %-3.3s" % \
              (
                  queue.priority,
                  queue.nice,
                  ",".join([s.friendly for s in queue.statuses]),
                  "-" if queue.max_slots == 2147483647 else queue.max_slots,
...
def print_short():
    ...
    for queue in Queue.get_queues_by_names(connection, args.queue_names):
        ...
        if args.host and queue.allowed_hosts and args.host not in args.allowed_hosts:
            continue
        print "%-15.15s %-4.4s %-15.15s %-3.3s %-4.4s %-4.4s %-4.4s %-7.7s %-6.6s %-4.4s %-4.4s" % (
            queue.name,
            queue.priority,
            ",".join([s.friendly for s in queue.statuses]),
            "-" if queue.max_slots == 2147483647 else queue.max_slots,
...
def print_wide():
    ...
    for queue in Queue.get_queues_by_names(connection, args.queue_names):
        ...
        print "%-15s %-4s %-15s %-3s %-4s %-4s %-4s %-7s %-6s %-4s %-4s" % (
            queue.name,
            queue.priority,
            ",".join(
                [s.friendly for s in queue.statuses]),
            "-" if queue.max_slots == 2147483647 else queue.max_slots,
            "-" if queue.max_slots_per_user == 2147483647 else queue.max_slots_per_user,
            "-" if queue.max_slots_per_processor == 2147483648.0 else queue.max_slots_per_processor,
            "-" if queue.max_slots_per_host == 2147483647 else queue.max_slots_per_host,
...
parser.add_argument("queue_names", default=None, nargs="*",
                    help="Only displays information about the specified queue or queueus")
parser.add_argument("-u", default=None, dest="user", type=str,
                    help="Displays the queues that can accept jobs from the specified user or user group.  \
                    If the keyword `all' is specified, displays the queues that can accept jobs from all \
                    users.")
parser.add_argument("-m", default=None, dest="host", type=str,
                    help="Displays the queues that can run jobs on the specified host or host group.  \
                    If the keyword all is specified, displays the queues that can run jobs on all  hosts")
...
try:
    ...
    print "Unable to display queue information: %s" % e.message
    sys.exit(1)