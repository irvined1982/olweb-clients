...
def print_short():
    ...
    for host in Host.get_hosts_by_names(connection, args.hostnames):
        print "%-18.18s %-12.12s %-7.7s %-4.4s %-8.8s %-4.4s %-6.6s %-8.8s %-4.4s" % \
              (host.host_name,
               ",".join([s.friendly for s in host.statuses]),
               "-" if host.max_slots_per_user == 2147483647 else host.max_slots_per_user,
               host.max_slots, host.total_slots, host.num_running_slots, host.num_system_suspended_slots,
               host.num_user_suspended_slots, host.num_reserved_slots)


def print_wide():
    print "HOST_NAME          STATUS       JL/U    MAX  NJOBS    RUN  SSUSP  USUSP    RSV"
    for host in Host.get_hosts_by_names(connection, args.hostnames):
        print "%-18s %-12s %-7s %-4s %-8s %-4s %-6s %-8s %-4s" % (
            host.host_name,
            ",".join([s.friendly for s in host.statuses]),
            "-" if host.max_slots_per_user == 2147483647 else host.max_slots_per_user, host.max_slots, host.total_slots,
            host.num_running_slots, host.num_system_suspended_slots, host.num_user_suspended_slots,
...
parser.add_argument("-w", action='store_true', dest="wide", help="Displays host information in wide format. \
Fields are displayed without truncation.")
parser.add_argument("-l", action='store_true', dest="long", help="Displays  host  information in a (long) multi-line \
format. In addition to the default fields, displays information about the CPU factor, the dispatch windows, the \
current load, and the load thresholds.")
parser.add_argument("hostnames", default=None, nargs="*", help="Only  displays  information  about the specified \
hosts or host groups. For host groups")
...
try:
    ...
    print "Unable to display host information: %s" % e.message
    sys.exit(1)
